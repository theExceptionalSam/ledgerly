...
export default function AgedDebtors() {
  ...
  useEffect(() => {
    ...
    api.get(`/aged-debtors?termId=${selectedTermId}`)
      .then((d) => setData(d))
  ...
  return (
    <div>
      <h1>Aged Debtors</h1>
      <p className="page-intro">Outstanding fees bucketed by age. Use this to prioritize collection efforts.</p>

      <div className="term-switcher">
        <label htmlFor="aged-term">Term</label>
        <select id="aged-term" name="termId" value={selectedTermId || ""} onChange={(e) => setSelectedTermId(e.target.value)}>
      ...
      {!loading && (
        <>
          ...
          <div className="card">
            <div className="card-title">Total Outstanding: {naira(total)}</div>
          </div>

          {students.length > 0 && (
            <div className="card">
              <div className="card-title">Student Breakdown</div>
              <div className="table-wrapper">
                <table className="fee-table">
                  <thead>
                    <tr><th>Student</th><th>Class</th><th className="num">Expected</th><th className="num">Paid</th><th className="num">Outstanding</th><th>Days Overdue</th><th>Bucket</th></tr>
                  </thead>
                  <tbody>
                    {students.map((s) => (
                      <tr key={s.studentId}>
                        ...
                        <td>{s.daysOverdue}</td>
                        <td><span className="badge" style={{ color: s.bucket === "0-30" ? "#1B7A43" : s.bucket === "90+" ? "#B3261E" : "#C77D22", background: s.bucket === "0-30" ? "#E6F4EA" : s.bucket === "90+" ? "#FBEAE9" : "#FBF0E2" }}>{s.bucket}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {students.length === 0 && !loading && <div className="empty-state">No outstanding fees for this term. 🎉</div>}
        </>
      )}
    </div>
  );
